			...
			if(isset($obj['Conditions'])){
				for($i=0; $i < $maxcond; $i++){
					if(isset($cond[$i])){
						...
						$maxprobcondition = ($condprob > $maxprob)?$conditions['ConditionName']:$maxprobcondition;
						$maxcond = ($condprob > $maxprob)?$condprob:$maxprob;
      ...
      foreach($obj['Symptoms'] as $symptoms){
        if(!in_array($symptoms['SymptomID'], $key_array)){
          ...
          if(!is_null($symptoms['SymptomID']) && $symptoms['SymptomID'] != ""){
			...
			$query = $db->prepare("SELECT Hint FROM conditions WHERE ConditionName = :ConditionName");
			$query->bindParam(":ConditionName", $maxprobcondition, PDO::PARAM_STR);
			$query->execute();
			$que = $query->fetch();
			if($que){
				$response['Hint'] = $que['Hint'];
			}
