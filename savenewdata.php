	...
	try
		{
			...
			if(empty($obj['ClinicID'])){
				$query = $db->prepare("SELECT cd.ClinicID FROM clinicdoctors cd INNER JOIN doctorprofile dp ON dp.CurrClinicID = cd.ClinicID WHERE cd.DID = dp.DID AND dp.DID = :DID");
        ...
        $doctorName =  "Dr. ".$row5['FName']." ".$row5['LName'];
				...
				$row = $query2->fetch();
				if($row['FName'] = "User"){
				        $result0 = $db->prepare("UPDATE user SET FName = :FName WHERE UserID = :UserID");
                                        $result0->bindParam(":FName", $obj['Name'], PDO::PARAM_STR);
                                        $result0->bindParam(":UserID", $row['PID'], PDO::PARAM_INT);
                                        $result0->execute();
				}else{
      ...
      if($count4 == 0){
			...
			if(isset($obj['Height']) && is_null($que5['Height'])){
				$upstat.=", Height = :Height";
			}
			if(isset($obj['Weight']) && is_null($que5['Weight'])){
				$upstat.=", Weight = :Weight";
			}
			if(isset($obj['BloodGroup']) && is_null($que5['BloodGroup'])){
				$upstat.=", BloodGroup = :BloodGroup";
			}
			if(isset($obj['Allergies']) && is_null($que5['Allergies'])){
				$upstat.=", Allergies = :Allergies";
			}
			if(isset($obj['Hereditory']) && is_null($que5['Hereditory'])){
				$upstat.=", Hereditory = :Hereditory";
			}
			$upstat.=" where PID=:PID";
			$result5 = $db->prepare($upstat);
